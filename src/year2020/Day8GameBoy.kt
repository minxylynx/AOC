...
//                println(running)
        ...
        fun switch(trip: Triple<String, String, Boolean>, accum: Int): Pair<Int, String>? =
            when (trip.first) {
                "acc" -> Pair(accum + trip.second.toInt(), "+1")
                "jmp" -> Pair(accum, trip.second)
                "nop" -> Pair(accum, "+1")
                else -> null
            }

        fun findTheEnd() {
            val tripSet = dataSet1.map { inst ->
                inst.split(" ")
                    .let { QueueSpot(it[0], it[1].toInt(), false, false) }
            }.toMutableList()


        }

        data class QueueSpot(val inst: String, val value: Int, var alreadyHit: Boolean, var opposite: Boolean)

        var runningMore = 0
        fun hopUpAndDownAgain(index: Int, list: MutableList<QueueSpot>) {
            var trip = list[index]
            println("${trip.inst} ${trip.value}")
            if (!trip.alreadyHit) {
                val pair = switch2Point0(trip, runningMore)
                runningMore = pair!!.first
                list[index] = trip.copy(alreadyHit = true)
//                println(list[index])
//                println(running)
                hopUpAndDownAgain(index + pair.second.toInt(), list)
            } else {
                println("trying things")
                println(runningMore)
            }
        ...
        fun switch2Point0(spot: QueueSpot, accum: Int): Pair<Int, Int>? =
            when (spot.inst) {
                "acc" -> Pair(accum + spot.value, 1)
                "jmp" -> Pair(accum, spot.value)
                "nop" -> Pair(accum, 1)
                else -> null
            }
        ...
        private fun backtrackPath(
            ...
            val path: MutableList<QueueSpot> = ArrayList<QueueSpot>()
            var iter: QueueSpot = cur
            while (iter != null) {
                path.add(iter)
            }
            return path
